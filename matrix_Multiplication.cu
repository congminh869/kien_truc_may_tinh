 ...
 */
void print_result(float* matrix, char* file_Name, int x_dim, int y_dim, int dim, float et, double time_spent)
{
    ...
    outFile << x_dim << "x" <<y_dim << ", " <<et << ", " << time_spent<<"\n";
...
void calculate_matrix(int Left_matri_x,int Left_matri_y,int Right_matri_x,int Right_matri_y)
{
    //size of the vectors to be processed  and matrix dimensions
    int Left_matrix_x, Left_matrix_y, Right_matrix_x, Right_matrix_y;
    
    Left_matrix_x = Left_matri_x;
    Left_matrix_y = Left_matri_y;
    Right_matrix_x = Right_matri_x;
    Right_matrix_y = Right_matri_y;
    ...
    //commented out the functions which helps to calculate time
    //printf("kich thuoc ma tran la %d x %d \n", Left_matrix_x, Left_matrix_y);
    //printf("GPU time= %f ms\n", et);
     
    //printf("CPU time= %lf ms\n", time_spent);
    printf("%dx%d, %f, %lf", Left_matrix_x, Left_matrix_y, et, time_spent);
    //cout<<Left_matrix_x << " " <<Left_matrix_y << " " <<et << " " << time_spent<<"\n";
    //print_results(Res_h,"GPU_out",Left_matrix_x,Right_matrix_y,dim);

    //Prints the results
    print_result(Res_h,"output",Left_matrix_x,Right_matrix_y,dim, et, time_spent);
...
int main() {
	...
	for (int j = 0; j < n; j++){
    	...
    	for (int j = 0; j < n; j++){
    		calculate_matrix(matrix[j][0], matrix[j][1], matrix[j][2], matrix[j][3]); 
    		//cout<< c <<"\n";
    		//myfile << matrix[j][0] << matrix[j][0] << "\n";
